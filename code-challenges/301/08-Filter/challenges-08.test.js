...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  // return arr.filter((a)=>{
  //   if(!forbiddenValues.includes(a)){
  //     return a;
  //   }
  // })
  return arr.filter(function(n){ return forbiddenValues.indexOf(n)>-1?false:n;});
};
...
const getStatName = (arr, minBaseStat) => {
  let valFilter = arr.filter(stats => stats.baseStat > minBaseStat);
  return valFilter.map(stats => stats.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(val => !val.children ? val : null);
  };
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  return arr.filter(val => parseInt(val) === val).map(val => val % 2 === 0? 'even' : 'odd');
};
