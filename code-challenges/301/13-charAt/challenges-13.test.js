...
const firstLetters = (arr) => {
  // Solution code here...
  return arr.map(elem => elem.charAt(0))
};
...
// return arr.map(num => num.split('').filter(str => (/\d/g).test(str)? str : null).join(''));
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  return str.split('').filter((elem,idx) => idx % 2 ===1 ? elem : null).join('');
};
...
example, ['this is great :)', 'wow', 'whyyyyyy :(', ':)))))']
------------------------------------------------------------------------------------------------ */

const allHappy = (arr) => {
  let result = true;
  // arr.map(elem => elem.includes(':)')? result = false: result)
  // return result;
  arr.map((val) =>{
    if(!val.includes(':)')){
      result = false;
...
const findAnything = (arr, target) => {
  // Solution code here...
  return arr.filter(elem => elem.includes(target)? elem : null)
};
...
const characterByIndex = (arr) => {
  // Solution code here...
  return arr.map((elem, idx) => elem.charAt(idx))
};
