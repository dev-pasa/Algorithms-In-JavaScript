...
const returnNames = (arr) => {
  return arr.reduce((val, n) => val.concat([n.name]), [])
};
...
const reversedString = (arr) => {
  return arr.split('').reduce((front, back) => back + front)
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((val, obj) => val + (obj.children ? obj.children.length : 0),0)
};
...
------------------------------------------------------------------------------------------------ */
// let arr = [18, 290, 37, 4, 55, 16, 7, 85 ] should return 64
const calculateAverage = (arr) => {
  return arr.reduce((sum, count) => sum + count/arr.length, 0)
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((vals, num) => isPrime(num) ? vals + 1: vals, 0)
}
