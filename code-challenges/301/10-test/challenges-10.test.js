...
const count = (target, input) => {
  let allNums = input.reduce((a,b) => a.concat(b), []);
  let targetTimes = allNums.filter((num) => num === target);
  return targetTimes.length;
};
...
const totalSum = (input) => {
  let allNums = input.reduce((a,b) => a.concat(b), []);
  let allSum = allNums.reduce((x, y) => x + y);
  return allSum;
};
...
let findMaleAndFemale = (data) => {
  return data.reduce((val, n) => !val.gender =="n/a" ? val.concat([n.name]) : null , [])
}
