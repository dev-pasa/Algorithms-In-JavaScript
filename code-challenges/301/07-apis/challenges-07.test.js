...
If any element in the array is not a number, the resulting array should have the string "N/A" in its place.[5, 8, 2, 'hi']
...
const extractAbilities = (arr) => {
  let newArr = arr.map(val => val.ability.name);
  return newArr;
...
//   const newLocal = arr.map((val) => {
//     val.stats.name, (parseInt(val.effort) + parseInt(val.baseStat));
//   });
// return newLocal

  let newArr = arr.map(val => {
      let newObj = new Object;
      newObj.name = val.stat.name;
      newObj.total = (val.baseStat) + (val.effort);
      return newObj;
  })
  return newArr;
 };
